...
<template>
  ...
  >
    ...
    <div>
      <div v-for="item in menuItems" :key="item.id" class="w-full mb-2 flex flex-col gap-2">
        <router-link :to="item.href"  :class="
                route.path === item.href ? 'bg-white text-[#242290] px-6 py-2 w-full flex place-items-center gap-6 rounded-s-full' : 'bg-[#242290] flex place-items-center px-6 py-2 gap-6'
              ">
            <component
              :is="item.icon"
              :class="
                route.path !== item.href ? 'text-[#C1BBEB] ' : 'text-[#4D44B5]'
              "
