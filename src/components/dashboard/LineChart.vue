...
import { Chart, LinearScale, CategoryScale, LineElement, PointElement, LineController, Title, Tooltip, Legend } from 'chart.js';
...
onMounted(() => {
        ...
        datasets: [
          {
            ...
            data: [30, 25, 80, 65, 30, 45, 82, 88, 67, 38, 34, 40], // Data for first line

            borderColor: ['#FB7D5B'], // Line color
            backgroundColor:['#FB7D5B'],
            tension: 0.3, // Line smoothing (0.1 is a slight curve)
            pointBackgroundColor: '#FB7D5B', // Point color
            fill: true,

            // Hide all points by default
            // Enable the point for the current month
  ...
  data: [0, 85, 45, 70, 28, 25, 30, 52, 42, 78, 85, 45], // Data for second line
  // fill: 0, // Do not fill area under the line
  borderColor: ['#FCC43E'], // Line color for the second dataset
  ...
  plugins: {
    legend: {
      ...
      align: 'end',  // Align the legend to the right (end of the top)
      labels: {
        ...
        boxWidth: 20,  // Width of the box next to the label (the colored square representing the dataset)
      }
    },
  },
  scales: {
...
<template>
  <div class="flex flex-col boredr-2 w-full bg-white rounded-xl">
