...
const calendarDays = computed(() =>
  generateDate(selectedMonth.value, today.value.year()),
)
...
function selectDay(day: dayjs.Dayjs) {
  if (
    selectedDays.value.length < 3 &&
    !selectedDays.value.some(d => d.isSame(day, 'day'))
  ) {
...
function getDayClass(day: CalendarDay) {
  ...
  let baseClasses =
    'max-h-[50px] w-full h-full max-w-[50px] grid place-items-center justify-center rounded-full font-[400] text-[18px] leading-[27px] text-[#303972]'
...
<template>
  <div class="w-full bg-white p-5 flex-1 rounded-lg">
    ...
    <div
      class="md:flex md:justify-between w-full md:px-5 sm:flex sm:flex-col sm:gap-2 md:flex-row sm:mb-5 md:mb-5"
    >
      <div class="font-[700] header leading-[36px] text-[#303972]">
        School Calendar
      </div>
      <div class="relative flex place-items-center">
        <select
          ...
          class="w-fit focus:outline-none font-[400] text-[18px] leading-[27px] text-[#303972] custom-select"
        >
        ...
        <span class="">
          <component
            class="text-[#716f74] md:absolute md:top-[50%] md:right-[-20%] md:translate-y-[-50%] sm:top-[-50%]"
            :is="dropDown"
          />
        </span>
      </div>
    ...
    <!-- Days of the week -->
    <div
      class="grid grid-cols-7 w-full place-items-center leading-[27px] text-[#A098AE] mb-2 font-[400] text-[18px] border-gray-300"
    >
      <div
        v-for="(day, index) in days"
        :key="index"
        class="font-[700] px-2 py-1"
      >
        {{ day }}
      </div>
    </div>
    ...
    <!-- Calendar days -->
    <div
      class="w-full h-[300px] grid grid-cols-7 gap-0 border-t border-l border-gray-300"
    >
      <!-- Each day block -->
      <div
        v-for="(day, index) in calendarDays"
        :key="index"
        class="grid place-items-center border-r border-b border-gray-300"
      >
