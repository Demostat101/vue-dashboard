...
// Filter teachersData based on searchName
...
<template>
  <div
    class="flex gap-5 flex-wrap 2xl:justify-left sm:justify-center md:justify-stretch flex-1"
  >
    <div class="flex-grow" v-for="teacher in slicedData" :key="teacher.id">
      <div
        class="flex flex-col text-center place-items-center py-3 gap-4 bg-white rounded-lg 2xl:min-w-[300px] sm:min-w-[200px] relative"
      >
        <div
          class="bg-[#C1BBEB] 2xl:w-[60px] 2xl:h-[60px] sm:w-[40px] sm:h-[40px] rounded-full"
        ></div>
        <div
          class="font-[700] 2xl:text-[24px] 2xl:leading-[36px] sm:text-[18px] sm:leading-[27px] text-[#303972] w-full text-nowrap"
        >
          {{ teacher.name }}
        </div>
        <div
          class="font-[400] 2xl:text-[18px] 2xl:leading-[27px] sm:text-[14px] sm:leading-[21px] text-[#A098AE]"
        >
          {{ teacher.subject }}
        </div>
